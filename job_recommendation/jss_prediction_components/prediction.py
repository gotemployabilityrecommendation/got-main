...
class Prediction:
    ...
    layer_1 = "job_recommendation/models/layer-1.pkl"
    layer_2_major = "job_recommendation/models/layer-2-major.pkl"
    layer_2_minor = "job_recommendation/models/layer-2-minor.pkl"
    layer_3_acc = "job_recommendation/models/layer-3-major-acc-fin.pkl"
    layer_3_admin = "job_recommendation/models/layer-3-major-admin.pkl"
    layer_3_it = "job_recommendation/models/layer-3-major-comp-it.pkl"
    layer_3_sales = "job_recommendation/models/layer-3-major-sales.pkl"
    layer_3_edu = "job_recommendation/models/layer-3-minor-edu.pkl"
    layer_3_media = "job_recommendation/models/layer-3-minor-media.pkl"
    layer_3_engg = "job_recommendation/models/layer-3-minor-engineering.pkl"
    ...
    def _predict_layer_3_minor_media(self, data_frame):
    ...
    def _convert_original_data(self, data):
        df1 = pd.read_csv("job_recommendation/data/for-reco-2021.csv")
        df2 = pd.read_csv("job_recommendation/data/for-reco-2022.csv")
    ...
    def _get_percentages(self, faculty_domain, jss):
        df1 = pd.read_csv("job_recommendation/data/for-reco-2021.csv")
        df2 = pd.read_csv("job_recommendation/data/for-reco-2022.csv")
        df = pd.concat([df1, df2], axis=0)

        data_group_domain = df.groupby(['Faculty_domain'])['Campus'].count()
        data_group_dict_domain  = data_group_domain.to_dict()
        data_group_domain_keys = data_group_dict_domain .keys()
        data_group_domain_keys = list(data_group_dict_domain .keys())
        data_group_domain_values = list(data_group_dict_domain .values())
        highlight_label_domain  = faculty_domain  # The label of the slice to highlight
        labels = np.array(data_group_domain_keys)
        data = np.array(data_group_domain_values)
        # print(highlight_label_domain)
        highlight_index = np.where(labels == highlight_label_domain )[0][0]
        highlight_data_count_domain  = data[highlight_index]
        
        data_group = df.groupby(['JS_Job_sub_sector','Faculty_domain'])['Campus'].count()
        
        data_group_dict = data_group.to_dict()
        data_group_value = data_group_dict[(jss,faculty_domain)]
        # print(data_group_keys)
        ...
        # print(data_group_value,highlight_data_count_domain)
        percentages = (100*int(data_group_value))//highlight_data_count_domain
        return percentages
    ...
    def predict(self, data):
        ...
        percentage = self._get_percentages(data['Faculty domain'], layer_3_js)
        return (layer_2_js, layer_3_js, percentage, data['Faculty domain'])
